...
import {
  ...
  LWM2M_TYPE,
  ACL,
} from './object-common.js';
import securityObject from './object-security.js';
import serverObject from './object-server.js';
import aclObject from './object-acl.js';
import deviceObject from './object-device.js';

const debug = debugLogger('node-red-contrib-lwm2m:lwm2m-common');

// Direct path to wakatiwaiclient binary (no longer using node-pre-gyp)
const CLIENT_PATH = path.resolve(__dirname, '..', 'build');
const WAKATI_CLIENT_PATH = path.join(CLIENT_PATH, 'wakatiwaiclient');

// Check if wakatiwaiclient binary exists and log appropriate message
function checkWakatiClient() {
  try {
    if (fs.existsSync(WAKATI_CLIENT_PATH)) {
      debug('Found wakatiwaiclient binary at:', WAKATI_CLIENT_PATH);
      return true;
    } else {
      console.error('LwM2M Plugin Error: wakatiwaiclient binary not found at expected location:', WAKATI_CLIENT_PATH);
      console.error('Please ensure the wakatiwaiclient binary is built and placed in the /build directory');
      return false;
    }
  } catch (error) {
    console.error('LwM2M Plugin Error: Failed to check for wakatiwaiclient binary:', error.message);
    return false;
  }
}
...
export class LwM2MClientProxy extends EventEmitter {
  ...
  start() {
    // Check if wakatiwaiclient binary exists before attempting to start
    if (!checkWakatiClient()) {
      throw new Error('wakatiwaiclient binary not found. Cannot start LwM2M client.');
    }

    let processPath = WAKATI_CLIENT_PATH;
    const args = [];
